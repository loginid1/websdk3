...
import { ApiError, AuthCode, DeviceInfo, Passkey } from "@loginid/core/api";
...
export interface AuthenticateWithPasskeysOptions extends MainPasskeyOptions {
  ...
  /**
   * Transaction-specific payload used to initiate a non-repudiation signature process.
   *
   * When provided, the payload is embedded in the passkey challenge and signed by the
   * user with their passkey. This can be used to authorize sensitive actions, such as
   * payments or changes to account information, and provides evidence that the
   * transaction was approved by the passkey holder.
   */
  txPayload?: string;

  /**
   * Optionally used with `txPayload`. A unique nonce that helps ensure the integrity
   * of the transaction and prevents replay attacks.
   */
  txNonce?: string;
...
export interface AuthResult {
  ...
  /**
   * Information about the passkey involved in this authentication session,
   * including passkey creation or authentication results when applicable.
   *
   * Available only after the session is complete.
   */
  passkeyCredential?: Passkey;

  /**
   * The unique identifier of the confirmed transaction.
   *
   * Available only after a transaction has been successfully confirmed with a passkey.
   */
  txId?: string;
}
