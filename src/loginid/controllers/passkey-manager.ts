...
class PasskeyManager extends LoginIDBase {
   ...
   * ```
   */
  async renamePasskey(id: string, name: string, options: RenamePasskeyOptions = {}): Promise<void> {
    const token = this.session.getToken(options)
   ...
   * ```
   */
  async deletePasskey(id: string, options: DeletePasskeyOptions = {}): Promise<void> {
    const token = this.session.getToken(options)
