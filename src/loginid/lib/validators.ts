...
import { PublicKeyCredentialCreationOptions, PublicKeyCredentialRequestOptions } from '../../api';
import { Complete, LoginIDConfig, MfaFactorName, MfaInfo, MfaOtpFactorName, MfaPerformFactorOptions } from '../types'
...
class LoginIDParamValidator {
  ...
  public static checkValidMfaOtpFactorName(factorName: MfaOtpFactorName) {
    if (factorName !== "otp:email" && factorName !== "otp:sms") {
      new LoginIDError(`MFA factor ${factorName} is not supported in the current MFA request flow.`)
    }
  }
