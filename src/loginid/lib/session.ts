...
import { Mfa } from '@/src/api'
import LoginIDConfigValidator from './validators'
import { LoginIDConfig, LoginIDTokenSet, PasskeyOptions, SessionInfo } from '../types'
import { 
  deleteCookie,
  getCookie,
  parseJwt,
...
class SessionManager {
  ...
  public getSessionInfo(): SessionInfo | null {
    ...
    const data = parseJwt(this.getJwtCookie() || this.retrieveToken("accessToken") || '{}')
  ...
  */
  ...
  public setTokenSet(result: Mfa) {
    ...
    const setTokenCookie = (name: string, token: string) => {
      ...
      const expiry = tokenPayload?.exp ? new Date(tokenPayload.exp * 1000).toUTCString() : "";
   ...
   */
  public retrieveToken(tokenType: 'idToken' | 'accessToken' | 'refreshToken' | 'payloadSignature'): string {
    const tokenNameMap = {
