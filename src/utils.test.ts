...
describe('bufferToBase64Url', () => {
  ...
  it('should convert a Fido2 rawId buffer to base64url', () => {
    // Create a sample ArrayBuffer.
    const buffer = new Uint8Array([187, 106, 173, 124, 221, 133, 51, 143, 84, 212, 28, 114, 228, 123, 6, 24, 133, 55, 48, 93, 77, 65, 179, 95, 88, 189, 253, 145, 33, 149, 144, 86]).buffer // "u2qtfN2FM49U1Bxy5HsGGIU3MF1NQbNfWL39kSGVkFY" in ASCII
...
describe('base64UrlToBuffer', () => {
  it('should convert a base64 URL string to a buffer', () => {
    ...
    expect(new Uint8Array(result)).toEqual(new Uint8Array([72, 101, 108, 108, 111])) // Ensure the buffer content matches.
  })
  ...
  it('should handle a Fido2 challenge', () => {
    // Call the base64UrlToBuffer function with the mocked dependencies.
    const result = base64UrlToBuffer('VaVQKNxmdTAlSoNb25L_ndhnNN6rb7rvZvzqgdAgsiM')

    // Expectations
    expect(result instanceof ArrayBuffer).toBe(true) // Ensure the result is an ArrayBuffer.
    expect(new Uint8Array(result)).toEqual(new Uint8Array([85, 165, 80, 40, 220, 102, 117, 48, 37, 74, 131, 91, 219, 146, 255, 157, 216, 103, 52, 222, 171, 111, 186, 239, 102, 252, 234, 129, 208, 32, 178, 35])) // Ensure the buffer content matches.
  })
  ...
  it('should handle a Fido2 user ID', () => {
    ...
    // Expectations
    expect(result instanceof ArrayBuffer).toBe(true) // Ensure the result is an ArrayBuffer.
    expect(new Uint8Array(result)).toEqual(new Uint8Array([177, 178, 142, 26, 16, 184, 167, 177, 223, 125, 223, 237, 198, 63, 100, 159])) // Ensure the buffer content matches.
  })
})
